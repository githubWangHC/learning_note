...
int squeeze(char str_input[], char str_dele_input[])
{
	...
	{
/*
		while((str_input[j++] == str_dele_input[k++]) && (str_dele_input[k-1] != '\0'))

*/
		int j = i, same_length = 0;
		while(str_dele_input[same_length] != '\0' && str_input[j] == str_dele_input[same_length])
		{
			j++;
			same_length++;
		}
		if(str_dele_input[same_length] == '\0')
		{
			int j = i;
			while((str_input[j] = str_input[j + same_length]) != '\0')
				j++;
		}
	}
}
